...
char **Response::makeCgiEnv(void)
{
	...
	size_t idx;
	std::map<std::string, std::string> &headersMap = m_client->get_m_request().get_m_headersMap();

	std::string &reqlocation = get_m_client()->get_m_request().get_m_reqlocation(); //"/"
	...
	cgi_map["CONTENT_LENGTH"] = std::to_string(get_m_client()->get_m_request().get_m_body().size());
	...
	if (m_cgi_extension == ".bla")
		...
		cgi_map["REQUEST_URI"] = m_client->get_m_request().get_m_reqlocation();
	cgi_map["SCRIPT_EXEC"] = m_location->get_m_cgi()[m_cgi_extension];
	...
	cgi_map["SCRIPT_NAME"] = m_location->get_m_cgi()[m_cgi_extension];
...
{
	if (m_client->get_m_c_status() == MAKE_RESPONSE)
	{
		int read_fds[2];
		int write_fds[2];
		...
		{  
			...
			args[0] = strdup(m_location->get_m_cgi()[m_cgi_extension].c_str());
		...
		else 
		{
			...
			Config::get_m_config()->get_m_webserv()->change_events(Config::get_m_config()->get_m_webserv()->get_m_change_list(), \
									m_fd_read, EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, NULL);
			m_client->set_m_c_status(FILE_WRITING);

		}
	}
	else if (m_client->get_m_c_status() == FILE_READ_DONE)
	{
		close(m_fd_read);
