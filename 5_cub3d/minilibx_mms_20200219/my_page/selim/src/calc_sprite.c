...
/*   Updated: 2021/02/03 15:20:26 by selim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_sprite(t_game *g, int x, int height, int width)
{
	int draw;
	int color;
	int y;
	...
	g->ray_sprite.tex_x = (int)((256 * (x - (-g->ray_sprite.width / 2 + \
		g->ray_sprite.screen_x)) * g->config.tex[4].width\
		...
		x < g->config.screenwidth && g->ray_sprite.trans_y < g->z_buf[x])
	{
		...
		while (y < g->ray_sprite.draw_end_y)
		{
			...
			color = g->config.tex[4].texture[width * \
...
void	calc_sprite(t_game *game)
{
	...
	while (i < game->config.sprite_num)
	{
		...
		{
			get_sprite(game, stripe, game->config.tex[C_S].height, \
					game->config.tex[C_S].width);
			stripe++;
		}
		i++;
	}
