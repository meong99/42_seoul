...
{
	game->ray.char_pos_x = (double)(game->char_x * TILE_SIZE);
	game->ray.char_pos_y = (double)(game->char_y * TILE_SIZE);
	game->ray.dir_x = -1;
	game->ray.dir_y = 0;
	game->ray.plane_x = 0;
...
void	raycast(t_game *game)
{
	...
	while (++x < WIDTH)
	{
		...
		while (game->ray.hit == 0)
		{
			...
			if (game->img.data[game->ray.map_y * game->ray.map_x] == 0xFFFFFF)
				game->ray.hit = 1;
			else
				game->img.data[game->ray.map_y * game->ray.map_x] = 0xFF00FF;
		}
	}
}