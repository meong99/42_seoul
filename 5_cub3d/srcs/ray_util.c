...
void	texture_set(t_game *game, int x)
{
	...
	while (++y < game->ray.draw_end)
	{
		...
		game->ray.texture_color = game->img.data[TEX_HEIGHT * game->ray.tex_y + game->ray.tex_x];
		game->ray.buf[y][x] = game->ray.texture_color;
	}
	draw_image(game, game->ray.buf);
	int_free(game->ray.buf, 2);
}
